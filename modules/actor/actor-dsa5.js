...
export default class Actordsa5 extends Actor {
        ...
        try {
            ...
            if (data.type == "character" || data.type == "npc") {
                ...
                data.data.status.soulpower.value =
                    ...
                    Math.round(
                        (data.data.characteristics.mu.value +
                            data.data.characteristics.kl.value +
                            data.data.characteristics.in.value) /
                        6
                    )
                data.data.status.toughness.value =
                    (data.data.status.toughness.initial || 0) +
                    Math.round(
                        (data.data.characteristics.ko.value +
                            data.data.characteristics.ko.value +
                            data.data.characteristics.kk.value) /
                        6
                    )
            ...
            if (data.type == "creature") {
                ...
                data.data.status.initiative.value =
                    data.data.status.initiative.current + (data.data.status.initiative.modifier || 0)
            }
            ...
            data.data.status.dodge.value =
                Math.round(data.data.characteristics.ge.value / 2) + data.data.status.dodge.gearmodifier
    ...
    static armorValue(actor, options = {}) {
        ...
        const protection = wornArmor.reduce(
            (a, b) => a + EquipmentDamage.armorWearModifier(b.data, b.data.data.protection.value),
            0
        )
                ...
                switch (i.type) {
                    ...
                    case "armor":
                        ...
                        totalWeight += parseFloat(
                            (
                                i.data.weight.value *
                                (i.toggleValue ? Math.max(0, i.data.quantity.value - 1) : i.data.quantity.value)
                            ).toFixed(3)
                        )
            ...
            if (vals.length == 2) {
                if (!isNaN(vals[0])) {
                    if (
                        ...
                        itemModifiers[vals[1]].value +=
                            Number(vals[0]) * (i.data.data.step ? Number(i.data.data.step.value) || 1 : 1)
    ...
    async applyRegeneration(LeP, AsP, KaP) {
        const update = {
            "data.status.wounds.value": Math.min(
                this.data.data.status.wounds.max,
                this.data.data.status.wounds.value + (LeP || 0)
            ),
    ...
    async fateisTalented(infoMsg, cardOptions, newTestData, message, data) {
        ...
        renderTemplate("systems/dsa5/templates/dialog/isTalentedReroll-dialog.html", {
                        ...
                        callback: async(dlg) => {
                            ...
                            if (diesToReroll.length > 0) {
                                ...
                                for (let k of diesToReroll) {
                                    ...
                                    changedRolls.push(
                                        `${attr}${newTestData.roll.terms[k * 2].results[0].result}/${
    ...
    async fatereroll(infoMsg, cardOptions, newTestData, message, data, schipsource) {
            ...
            renderTemplate("systems/dsa5/templates/dialog/fateReroll-dialog.html", {
                    testData: newTestData,
                    postData: data.postData,
                }).then((html) => {
                        new DSA5Dialog({
                                ...
                                this[`${data.postData.postFunction}`](
                                    { testData: newTestData, cardOptions },
    ...
    async fateImprove(infoMsg, cardOptions, newTestData, message, data, schipsource) {
        ...
        if (["spell", "liturgy", "ceremony", "ritual", "skill"].includes(rollType)) {
            renderTemplate("systems/dsa5/templates/dialog/fateImprove-dialog.html", {
                testData: newTestData,
                postData: data.postData,
            }).then((html) => {
                new DSA5Dialog({
        ...
        let dialogOptions = {
            ...
            callback: (html, options = {}) => {
                ...
                testData.situationalModifiers.push(
                    {
                        name:
                            game.i18n.localize("camplocation") +
                            " - " +
                            html.find('[name="regnerationCampLocations"] option:selected').text(),
    ...
    static _prepareMeleeWeapon(item, combatskills, actorData, wornWeapons = null) {
        ...
        if (skill) {
            ...
            if (item.data.guidevalue.value != "-") {
                let val = Math.max(
                    ...item.data.guidevalue.value.split("/").map((x) => Number(actorData.data.characteristics[x].value))
                )
